...
def save_args(args): #saves the arguments in a txt file in the output folder
    with open(join(args['output_folder_path'],'args.txt'), 'w') as f: #saving all the parameters in a file.
        dump(args,f,indent = 2) 


def generate_histogram(shift_vertical_list,shift_horizontal_list,args):
    image_file_path = join(args['output_folder_path'],'shifts_distribution.png') #creating the file path
